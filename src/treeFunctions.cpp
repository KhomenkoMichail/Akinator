...
void createGraphImageForDump (struct tree_t* tree, FILE* dumpFile, const char* nameOfTextGraphFile) {
    ...
    snprintf(graphvizCallCommand, sizeof(graphvizCallCommand), "dot -Tpng %s -o DUMPS/IMAGES/graph%d.png", nameOfTextGraphFile, graphImageCounter);
    system(graphvizCallCommand);
    fprintf(dumpFile, "Image:\n <img src=DUMPS/IMAGES/graph%d.png width=1000px>\n", graphImageCounter);

}


tree_t* akinatorTreeCtor(tree_t* tree, dump* dumpInfo) {
    assert(tree);
    assert(dumpInfo);
